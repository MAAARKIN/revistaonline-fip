...
public class LoginController {
	...
	public LoginController(Autenticador autenticador, Environment environment,
			...
			HttpServletRequest request, Freemarker freemarker) {
		this.result = result;
	...
	@Post
	public void enviarEmail(String username) throws EmailException, IOException, TemplateException {
		Usuario usuario = usuarioRepositorio.getUsuario(username);
		if (usuario == null) {
			Controllers.includeError(result, "Usuário não existe");
		...
		stringbuilder.append(request.getScheme()).append("://")
				...
				.append(token);
		String body = freemarker.use("send_mail").with("link", stringbuilder.toString()).getContent();
		mailer.send(this.environment.get("mail.subject"), body, usuario.getAutor().getEmail());
	...
	public void auth(Usuario usuario) {
		try {
			if (usuario.getLogin() == null
				...
				validator.add(new ValidationMessage(localization.getMessage("autenticacao.credencial.vazio"), 
			...
			if (credencial != null) {
				...
				result.include("errors",Arrays.asList(new ValidationMessage(localization.getMessage("autenticacao.credencial.erro"),
								localization.getMessage("autenticacao.credencial")))).redirectTo(IndexController.class).index();
			}
			...
			result.include("errors", Arrays.asList(new ValidationMessage(authex.getMessage(),
							localization.getMessage("autenticacao.credencial")))).redirectTo(IndexController.class).index();
		} catch (Exception e) {
			e.printStackTrace();
			result.include("errors", Arrays.asList(new ValidationMessage(localization.getMessage("autenticacao.credencial.erro"),
